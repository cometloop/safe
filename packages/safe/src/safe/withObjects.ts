import type { SafeResult, SafeResultObj, SafeInstance, SafeObjectInstance } from './types'
...
export function withObjects<T, E>(result: Promise<SafeResult<T, E>>): Promise<SafeResultObj<T, E>>
// Overload 3: Wrap a sync function returning SafeResult
export function withObjects<A extends unknown[], T, E>(fn: (...args: A) => SafeResult<T, E>): (...args: A) => SafeResultObj<T, E>
// Overload 4: Wrap an async function returning Promise<SafeResult>
export function withObjects<A extends unknown[], T, E>(fn: (...args: A) => Promise<SafeResult<T, E>>): (...args: A) => Promise<SafeResultObj<T, E>>
// Overload 5: Wrap a SafeInstance
export function withObjects<E, TResult>(instance: SafeInstance<E, TResult>): SafeObjectInstance<E, TResult>
// Implementation
export function withObjects(input: unknown): unknown {
  ...
  if (input instanceof Promise) {
    return input.then((result) => toObjectResult(result as SafeResult<unknown, unknown>))
  }
  ...
  if (typeof input === 'function') {
    return (...args: unknown[]) => {
      ...
      if (result instanceof Promise) {
        return result.then((r) => toObjectResult(r as SafeResult<unknown, unknown>))
      }
  ...
  if (typeof input === 'object' && input !== null && 'sync' in input && 'async' in input) {
    ...
    return {
      ...
      async: (fn: any, hooks?: any) => instance.async(fn, hooks).then(toObjectResult),
      ...
      allSettled: async (fns: any) => {
        const results = await instance.allSettled(fns) as Record<string, SafeResult<unknown, unknown>>
